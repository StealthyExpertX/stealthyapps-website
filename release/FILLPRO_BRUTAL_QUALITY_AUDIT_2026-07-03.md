...
- Removed the ring/halo treatment, floating nodes, duplicate product-diagram cues, and extra diagonal treatments; the hero now uses a quiet beveled studio backplate, a small abstract form-depth stack, one beveled FillPro brand token, one guided rectangular fill cursor, a safe-skip rail, a soft contact shadow, and one small warm accent.
...
- Bumped the website cache token to `fillpro-launch-v48` so live browsers receive the updated CSS, theme control, and hero scene assets.
- Rebuilt the Three.js hero one more time into a simpler edge-depth render: one beveled studio plate, a small abstract form stack, one beveled FillPro token, one guided rectangular fill cursor, one safe-skip rail, one contact shadow, and one warm accent.
