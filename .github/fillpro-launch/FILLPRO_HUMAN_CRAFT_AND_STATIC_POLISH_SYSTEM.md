# FillPro Human Craft and Static Polish System

Updated: 2026-06-14

This is the working quality bar for FillPro website copy, extension UI text, and static HTML/CSS/JS polish. Use it when writing a page, editing popup text, adding a state message, or reviewing a release.

## 1. FillPro Voice

FillPro should sound calm, practical, and specific. It is a privacy tool for repeated forms, so the voice should feel careful without sounding scared.

Write like this:

- Short sentence first when the idea matters.
- Use concrete nouns: profile, form, field, resume, upload, browser, page.
- Say what FillPro does and what it does not do.
...
| Generic copy | FillPro copy |
| --- | --- |
| Unlock the power of effortless form completion. | Save the fields you keep retyping. Fill them when a form gets long. |
| FillPro seamlessly handles all your form needs. | FillPro handles repeated profile fields. Passwords, cards, and one-time codes stay with tools built for them. |
...
| An unexpected error occurred. | FillPro hit a snag before it could fill this page. A short report helps us fix the site. |
...
- 1 longer sentence only when a boundary needs context: "The floating button asks for access to the current site only, and you can turn it off from FillPro settings."
...
- "No smart rules yet. Run FillPro on a tricky form, then save the label once."
- "No disabled sites. FillPro is available on pages you choose."
...
Use the existing FillPro pattern: a low-contrast grid and one pointer-reactive radial highlight. It is cheaper than canvas, works on static hosting, and is easy to disable.
...
FillPro should feel distinct because the product has a clear boundary:
...
Before shipping a FillPro copy or UI change, check:
