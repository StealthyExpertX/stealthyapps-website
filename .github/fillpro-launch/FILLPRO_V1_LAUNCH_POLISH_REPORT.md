# Skip Retyping v1.0.0 Launch Polish Report
...
- Baymard-style button work favors clear target states, obvious primary actions, and avoiding extra friction. Skip Retyping's CTAs are direct and large.
...
- Primary product page: `https://stealthyapps.com/skip-retyping/`.
- Redirect/support pages remain available, but `/skip-retyping/` is the link to use in stores.
...
- `fillpro/privacy/index.html`: rewrote privacy copy around plain user language. The core promise is profiles are saved inside the extension, not sent to a Skip Retyping profile server.
...
- Main page: `fillpro-site/skip-retyping/index.html`.
- Privacy page: `fillpro-site/skip-retyping/privacy/index.html`.
...
- Current GIF/poster: empty repeated form, Skip Retyping profile card, safe field fill, upload match, sensitive field left untouched, review-before-submit note.
...
This does not count as a normal Skip Retyping failure:
...
- Product page uses `/skip-retyping/` as canonical.
...
- Internal launch docs live under `.github/skip-retyping-launch/`, not as public website pages.
...
3. Clean `/skip-retyping/` navigation and product copy.
...
- Do not claim support for protected pages Skip Retyping cannot reach.
